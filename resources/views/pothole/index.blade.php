        ...
        <form id="pothole-search-form" class="space-y-4">
            ...
            <div class="flex items-center gap-8">
                @if (Auth::user()->isAdmin())
                    <x-form-field type="radio" name="admin-filter" text="Filters: " :options="['assigned-to-me', 'open-potholes']" />
                @endif
                <x-form-field type="dropdown" name="status" text="Status: "
                    optionFlex="{{ Auth::user()->isAdmin() && 'col' }}" :options="['all', 'open', 'assigned', 'in-progress', 'resolved', 'verified']" />
