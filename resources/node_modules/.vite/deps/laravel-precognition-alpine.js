...
var createValidator = (callback, initialData = {}) => {
  ...
  const validate = (name, value, config) => {
    if (typeof name === "undefined") {
      const only = Array.from((config == null ? void 0 : config.only) ?? (config == null ? void 0 : config.validate) ?? []);
...
// node_modules/laravel-precognition-alpine/dist/index.js
function dist_default(Alpine) {
  Alpine.magic("form", (el) => (method2, url, inputs, config = {}) => {
    const originalData = cloneDeep_default(inputs);
    const originalInputs = Object.keys(originalData);
    const state = Alpine.reactive({
      touched: [],
      valid: []
    });
    const validator = createValidator((client2) => client2[resolveMethod(method2)](resolveUrl(url), form.data(), config), originalData).on("validatingChanged", () => {
    ...
    const createForm = () => ({
      ...cloneDeep_default(inputs),
      data() {
        const newForm = cloneDeep_default(form);
      ...
      validate(name, config2) {
        ...
        if (typeof name === "undefined") {
          ...
          name = resolveName(name);
          validator.validate(name, get_default(form.data(), name), config2);
        }
        return form;
      },
      validating: false,
      ...
      reset(...names) {
        const original = cloneDeep_default(originalData);
        if (names.length === 0) {
          originalInputs.forEach((name) => form[name] = original[name]);
        } else {
          names.forEach((name) => set_default(form, name, get_default(original, name)));
        }
      ...
      async submit(config2 = {}) {
        return client[resolveMethod(method2)](resolveUrl(url), form.data(), resolveSubmitConfig(config2));
      },
      validateFiles() {
    ...
    syncWithDom(Alpine, el, resolveMethod(method2), resolveUrl(url), form);
    return form;
  });
}
var syncWithDom = (Alpine, el, method2, url, form) => {
  if (!(el instanceof Element && el.nodeName === "FORM")) {
    return;
  }
  syncSyntheticMethodInput(el, method2);
  syncMethodAttribute(el, method2);
  syncActionAttribute(el, url);
  addProcessingListener(Alpine, el, form);
};
var syncMethodAttribute = (el, method2) => {
  if (method2 !== "get" && !el.hasAttribute("method")) {
    el.setAttribute("method", "POST");
  }
};
var syncActionAttribute = (el, url) => {
  if (!el.hasAttribute("action")) {
    el.setAttribute("action", url);
  }
};
var syncSyntheticMethodInput = (el, method2) => {
  if (["get", "post"].includes(method2)) {
    return;
  }
  ...
  input.setAttribute("value", method2.toUpperCase());
};
var addProcessingListener = (Alpine, el, form) => el.addEventListener("submit", () => {
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=laravel-precognition-alpine.js.map
